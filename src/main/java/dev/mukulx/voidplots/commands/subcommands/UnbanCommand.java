...
public class UnbanCommand implements SubCommand {
    ...
    public UnbanCommand(@NotNull VoidPlots plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (args.length < 1) {
            plugin.getMessageManager().sendMessage(sender, "general.invalid-args",
                Map.of("usage", "/plot unban <player>"));
            return;
        }
        ...
        if (!plot.isDenied(target.getUniqueId())) {
            plugin.getMessageManager().sendMessage(sender, "plot.ban.not-banned",
        ...
        plugin.getMessageManager().sendMessage(sender, "plot.ban.unbanned",
            Map.of("player", target.getName()));
    }
}
