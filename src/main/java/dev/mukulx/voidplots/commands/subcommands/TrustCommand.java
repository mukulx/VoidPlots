...
public class TrustCommand implements SubCommand {
    ...
    public TrustCommand(@NotNull VoidPlots plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (!player.hasPermission("voidplots.trust")) {
        ...
        if (args.length < 1) {
            plugin.getMessageManager().sendMessage(sender, "general.invalid-args",
                Map.of("usage", "/plot trust <player>"));
            return;
        }
        ...
        if (plot.isTrusted(target.getUniqueId())) {
            plugin.getMessageManager().sendMessage(sender, "plot.trust.already-trusted",
        ...
        plugin.getMessageManager().sendMessage(sender, "plot.trust.success",
            Map.of("player", target.getName()));
    }
}
