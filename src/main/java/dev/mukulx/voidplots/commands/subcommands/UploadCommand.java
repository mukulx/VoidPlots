...
public class UploadCommand implements SubCommand {
    ...
    public UploadCommand(@NotNull VoidPlots plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (!player.hasPermission("voidplots.upload")) {
        ...
        if (args.length < 1) {
            plugin.getMessageManager().sendMessage(sender, "general.invalid-args",
                Map.of("usage", "/plot upload <schematic>"));
            sender.sendMessage("§7Available schematics: §e" + 
                String.join(", ", plugin.getSchematicManager().listSchematics()));
            return;
        }
        ...
        if (!plugin.getSchematicManager().schematicExists(fileName)) {
            plugin.getMessageManager().sendMessage(sender, "plot.schematic.not-found",
        ...
        sender.sendMessage("§eUploading schematic to plot...");
        sender.sendMessage("§7This may take a moment for large schematics.");
        
        // Run async to avoid blocking
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                plugin.getSchematicManager().uploadPlot(finalPlotId, worldName, finalFileName);
                
                Bukkit.getScheduler().runTask(plugin, () -> {
                    plugin.getMessageManager().sendMessage(sender, "plot.schematic.upload-success",
                        Map.of("file", finalFileName));
                });
            } catch (Exception e) {
                plugin.getLogger().severe("Failed to upload plot schematic: " + e.getMessage());
                e.printStackTrace();
                
                Bukkit.getScheduler().runTask(plugin, () -> {
                    plugin.getMessageManager().sendMessage(sender, "plot.schematic.upload-failed",
    ...
    public List<String> tabComplete(@NotNull CommandSender sender, @NotNull String[] args) {
        if (args.length == 1) {
            return Arrays.asList(plugin.getSchematicManager().listSchematics());
        }
        return List.of();
    }
}
