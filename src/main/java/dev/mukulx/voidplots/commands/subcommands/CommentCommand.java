...
public class CommentCommand implements SubCommand {
    ...
    public CommentCommand(@NotNull VoidPlots plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (!player.hasPermission("voidplots.comment")) {
        ...
        if (args.length < 1) {
            plugin.getMessageManager().sendMessage(sender, "general.invalid-args",
                Map.of("usage", "/plot comment <message>"));
            return;
        }
        ...
        // Cannot comment on own plot
        if (plot.isOwner(player.getUniqueId())) {
            plugin.getMessageManager().sendMessage(sender, "plot.comment.own-plot");
            return;
        }
        ...
        if (message.length() > 200) {
            plugin.getMessageManager().sendMessage(sender, "plot.comment.too-long");
            return;
        }
        ...
        plugin.getMessageManager().sendMessage(sender, "plot.comment.success");
    }
}
